...
#if defined(NDEBUG)
# error "Rafsen cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "Rafsen Signed Message:\n";
                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<uint256>& vHashTxnToUncache)
{
...

    {
        ...
        if (fLimitFree && nModifiedFees < GetRafsenMinRelayFee(tx, nSize, !fLimitFree))
        {
...
static bool ReadBlockOrHeader(T& block, const CDiskBlockPos& pos, const Consensus::Params& consensusParams, bool fCheckPOW)
{
    ...
    // Check the header
    if (fCheckPOW && block.hashPrevBlock != uint256() && !CheckAuxPowProofOfWork(block, consensusParams))
...
bool CheckTxInputs(const CChainParams& params, const CTransaction& tx, CValidationState& state, const CCoinsViewCache& inputs, int nSpendHeight)
{
        ...
        for (unsigned int i = 0; i < tx.vin.size(); i++)
        {
            ...
            if (coins->IsCoinBase()) {
                // Rafsen: Switch maturity at depth 145,000
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    // Rafsen: BIP30 has been active since inception
    ...
    // Rafsen: BIP16 has been enabled since inception
    ...
    CAmount blockReward = nFees + GetRafsenBlockSubsidy(pindex->nHeight, chainparams.GetConsensus(pindex->nHeight), hashPrevBlock);
...
bool CheckBlockHeader(const CBlockHeader& block, CValidationState& state, bool fCheckPOW)
{
    ...
    if (fCheckPOW && block.hashPrevBlock != uint256() && !CheckAuxPowProofOfWork(block, Params().GetConsensus(0)))
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const CBlockIndex* pindexPrev, int64_t nAdjustedTime)
{
    ...
    // Rafsen: Disallow AuxPow blocks before it is activated.
    ...
    // Rafsen: Version 2 enforcement was never used
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state, const CBlockIndex* pindexPrev)
{
    ...
    // Rafsen: We probably want to disable this
    int nLockTimeFlags = 0;
...
static bool AcceptBlockHeader(const CBlockHeader& block, CValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex)
{
    ...
    if (hash != chainparams.GetConsensus(0).hashGenesisBlock) {
        ...
        if (!CheckBlockHeader(block, state) && block.hashPrevBlock != uint256())
